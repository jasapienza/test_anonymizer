...
md`# Sector List Implementation in JavaScript`
)}
...
(function() {
  let {XY_TOLERANCE} = Constants;
	var _trunc_func = function(coord) {
		return coord;
	};
	const Coordinates = {
		truncByResolution : (c) => _trunc_func(c),
...
function _Point(Coordinates){return(
(function(){
	let {truncByResolution} = Coordinates;
	
	class Point {
		constructor(x,y) {
			this.x = truncByResolution(x);
			this.y = truncByResolution(y);
		}
		clone() {
...
(function() {
	...
	let {eqCoord,lsCoord,gtCoord,truncByResolution} = Coordinates;
	...
	class Ray {
		constructor(source,theta,ds,y_low = null) {
			...
			this.theta = theta;
			this.ds = ds;
			this.y_low = (y_low === null ? this.source.y : truncByResolution(y_low));
		...
		interY(y) {
			...
			return truncByResolution(this.source.x + this.theta.dx * (y - this.source.y) / this.theta.dy);
		}
		inter(other) {
...
(function () {
  ...
  let { eqCoord, eqPoint } = Coordinates;
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("Point")).define("Point", ["Coordinates"], _Point);
  ...
  main.variable(observer()).define(["tex","md"], _16);
  ...
  main.variable(observer()).define(["tex","md"], _25);
